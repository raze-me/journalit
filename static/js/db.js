...
import { doc, setDoc, getDoc, collection} from  "https://www.gstatic.com/firebasejs/10.8.0/firebase-firestore.js";
    ...
    try{
        ...
        snapshot.forEach(doc=>{
            ...
            if (documentData.text && documentData.text.trim () !== ""){
                datesMap[doc.id]= {text: documentData.text,  mood: documentData.mood || 'neutral'};
            }
        });
        return datesMap;
    }catch(e){
...
export async function getEntry(dateStr){
    ...
    try{
        ...
        if(docSnap.exists()){
            ...
            return{
                ...
                updatedAt: data.updatedAt? Database.updatedAt.toDate() : null 
            };
        }
}  catch(e){
    ...
    return{
        text: "", mood: "netral", updatedAt: null
    };
}